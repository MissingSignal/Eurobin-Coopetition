...
class RobotAgent:
    ...
    def callback(self, msg):
        if not self.robot_speaking:        
            ...
            current_state = self.robot.invoke(
                {"messages": [
                    SystemMessage(f"Given a prompt in the format: Pick Object 'O' at Location 'L_origin' in Kitchen 'K_origin' and place it on 'K_target' 'L_target'. Where O={{{', '.join(objects)}}}, L_target={{Dishwasher, Table, Cabinet}}, L_origin={{Dishwasher, Table, Cabinet, person}}, K_target={{DLR, KIT, INRIA}}, and K_origin={{DLR, KIT, INRIA}}. If you are not sure about one component, ask for confirmation. Never list to the user all the objects that can be grasped."),
            ...
            self.text_pub.publish(response)
    ...
    @tool
    def execute_plan(self, object: str, origin_location: str, origin_sublocation: str, target_location: str, target_sublocation: str):
        ...
        text2pub = (f"Moving {object} from {origin_location} {origin_sublocation} to {target_location} {target_sublocation}")
        ...
        response = self.call_service('/pick_service', PickService, object=object, position=position, orientation=orientation)
        if not response:
            return False
